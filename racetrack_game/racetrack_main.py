...
rt_name = "draha_01.png"  #Defaultní dráha
...
formula_icon = pyglet.resource.image("formule_2.png")  #formula_icon
window.set_icon(formula_icon)

#Let's load the car engine sound and other sounds:
engine_sound = pyglet.media.load('menu_selection.wav', streaming=False)  
#originally true engine sound, but somehow it sounded weirdly in pyglet
play_engine_sound = True
menu_selection_sound = pyglet.media.load('menu_selection.wav', streaming=False)
win_sound = pyglet.media.load("winfretless.ogg", streaming=False)
game_over_sound = pyglet.media.load("GameOver.wav", streaming=False)
